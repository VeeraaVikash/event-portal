...
$query = "SELECT p.status, COUNT(*) as count FROM proposals p JOIN users u ON p.user_id = u.id WHERE u.department = ? GROUP BY p.status";
...
$year_query = "SELECT MONTH(p.created_at) as month, COUNT(*) as count FROM proposals p JOIN users u ON p.user_id = u.id WHERE u.department = ? AND YEAR(p.created_at) = YEAR(CURRENT_DATE) GROUP BY MONTH(p.created_at)";
...
// Fetch Proposals List for Department
$proposals = [];
...
$approved_events = [];
$cal_query = "SELECT p.id, p.title, p.start_date, p.end_date FROM proposals p JOIN users u ON p.user_id = u.id WHERE u.department = ? AND p.status = 'Approved'";
...
require_once 'views/dashboard_hod.view.php';
